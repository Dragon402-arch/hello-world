...
- fine-tuning GPT2 example
  ...
  - 代码示例
    ...
    # default to left padding 改为左边填充，因此要使用右边最后一个token做预测
    ...
    gpt2_classificaiton_collator = Gpt2ClassificationCollator(
        use_tokenizer=tokenizer, labels_encoder=labels_ids, max_sequence_len=max_length
    )
    ...
    train_dataset = MovieReviewsDataset(
        path='/content/aclImdb/train', use_tokenizer=tokenizer
    )
    ...
    valid_dataset = MovieReviewsDataset(
        path='/content/aclImdb/test', use_tokenizer=tokenizer
    )
